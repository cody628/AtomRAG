...
@dataclass
class AtomRAG:
    """AtomRAG: Simple and Fast Retrieval-Augmented Generation."""

    working_dir: str = field( default_factory=lambda: f'./AtomRAG_cache_{datetime.now().strftime("%Y-%m-%d-%H:%M:%S")}' )
    ...
    def __post_init__(self):
        ...
        log_file = os.path.join(self.log_dir, "AtomRAG.log")
        set_logger(log_file)
        ...
        logger.debug(f"AtomRAG init with param:\n  {_print_config}\n")
    ...
    async def aquery(
        ...
        if param.mode in ["ours"] :
            ...
            response, retrieves = await ours_kg_query_experiment1(
                query, 
                self.atomic_entity_relation_graph,
                self.atomic_entities_vdb,
                self.chunks_vdb,
