...
TABLES = {
    ...
    "AtomRAG_DOC_CHUNKS": {
        "ddl": """
        CREATE TABLE AtomRAG_DOC_CHUNKS (
            `id` BIGINT PRIMARY KEY AUTO_RANDOM,
    ...
    "AtomRAG_GRAPH_NODES": {
        "ddl": """
        CREATE TABLE AtomRAG_GRAPH_NODES (
            `id` BIGINT PRIMARY KEY AUTO_RANDOM,
    ...
    "AtomRAG_GRAPH_EDGES": {
        "ddl": """
        CREATE TABLE AtomRAG_GRAPH_EDGES (
            `id` BIGINT PRIMARY KEY AUTO_RANDOM,
    ...
    "AtomRAG_LLM_CACHE": {
        "ddl": """
        CREATE TABLE AtomRAG_LLM_CACHE (
...
SQL_TEMPLATES = {
    # SQL for KVStorage
    "get_by_id_full_docs": "SELECT doc_id as id, IFNULL(content, '') AS content from src.AtomRAG_DOC_FULL WHERE doc_id = :id AND workspace = :workspace",
    "get_by_id_text_chunks": "SELECT chunk_id as id, tokens, IFNULL(content, '') AS content, chunk_order_index, full_doc_id from src.AtomRAG_DOC_CHUNKS WHERE chunk_id = :id AND workspace = :workspace",
    "get_by_ids_full_docs": "SELECT doc_id as id, IFNULL(content, '') AS content from src.AtomRAG_DOC_FULL WHERE doc_id IN ({ids}) AND workspace = :workspace",
    "get_by_ids_text_chunks": "SELECT chunk_id as id, tokens, IFNULL(content, '') AS content, chunk_order_index, full_doc_id from src.AtomRAG_DOC_CHUNKS WHERE chunk_id IN ({ids}) AND workspace = :workspace",
    ...
    "upsert_doc_full": """
        INSERT INTO AtomRAG_DOC_FULL (doc_id, content, workspace)
    ...
    """,
    "upsert_chunk": """
        INSERT INTO AtomRAG_DOC_CHUNKS(chunk_id, content, tokens, chunk_order_index, full_doc_id, content_vector, workspace)
    ...
    "entities": """SELECT n.name as entity_name FROM
        ...
        from src.AtomRAG_GRAPH_NODES WHERE workspace = :workspace) n
    ...
    "relationships": """SELECT e.source_name as src_id, e.target_name as tgt_id FROM
        ...
        from src.AtomRAG_GRAPH_EDGES WHERE workspace = :workspace) e
    ...
    "chunks": """SELECT c.id FROM
        ...
        from src.AtomRAG_DOC_CHUNKS WHERE workspace = :workspace) c
        WHERE c.distance>:better_than_threshold ORDER BY c.distance DESC LIMIT :top_k
    """,
    "has_entity": """
        SELECT COUNT(id) AS cnt from src.AtomRAG_GRAPH_NODES WHERE name = :name AND workspace = :workspace
    """,
    "has_relationship": """
        SELECT COUNT(id) AS cnt from src.AtomRAG_GRAPH_EDGES WHERE source_name = :source_name AND target_name = :target_name AND workspace = :workspace
    """,
    "update_entity": """
        UPDATE AtomRAG_GRAPH_NODES SET
    ...
    """,
    "update_relationship": """
        UPDATE AtomRAG_GRAPH_EDGES SET
    ...
    """,
    "insert_entity": """
        INSERT INTO AtomRAG_GRAPH_NODES(entity_id, name, content, content_vector, workspace)
        VALUES(:id, :name, :content, :content_vector, :workspace)
    """,
    "insert_relationship": """
        INSERT INTO AtomRAG_GRAPH_EDGES(relation_id, source_name, target_name, content, content_vector, workspace)
    ...
    "get_node": """
        ...
        from src.AtomRAG_GRAPH_NODES WHERE name = :name AND workspace = :workspace
    """,
    "get_edge": """
        ...
        from src.AtomRAG_GRAPH_EDGES WHERE source_name = :source_name AND target_name = :target_name AND workspace = :workspace
    """,
    "get_node_edges": """
        ...
        from src.AtomRAG_GRAPH_EDGES WHERE source_name = :source_name AND workspace = :workspace
    """,
    "node_degree": """
        SELECT COUNT(id) AS cnt from src.AtomRAG_GRAPH_EDGES WHERE workspace = :workspace AND :name IN (source_name, target_name)
    """,
    "upsert_node": """
        INSERT INTO AtomRAG_GRAPH_NODES(name, content, content_vector, workspace, source_chunk_id, entity_type, description)
    ...
    """,
    "upsert_edge": """
        INSERT INTO AtomRAG_GRAPH_EDGES(source_name, target_name, content, content_vector,
