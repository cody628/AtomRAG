...
class OllamaServerInfos:
    ...
    AtomRAG_NAME = "AtomRAG"
    AtomRAG_TAG = os.getenv("OLLAMA_EMULATING_MODEL_TAG", "latest")
    AtomRAG_MODEL = f"{AtomRAG_NAME}:{AtomRAG_TAG}"
    AtomRAG_SIZE = 7365960935  # it's a dummy value
    AtomRAG_CREATED_AT = "2024-01-15T00:00:00Z"
    AtomRAG_DIGEST = "sha256:AtomRAG"


ollama_server_infos = OllamaServerInfos()


# query mode according to query prefix (bypass is not AtomRAG quer mode)
    ...
    def setup_routes(self):
        ...
        async def get_tags():
            ...
            return OllamaTagResponse(
                models=[
                    {
                        "name": self.ollama_server_infos.AtomRAG_MODEL,
                        "model": self.ollama_server_infos.AtomRAG_MODEL,
                        "size": self.ollama_server_infos.AtomRAG_SIZE,
                        "digest": self.ollama_server_infos.AtomRAG_DIGEST,
                        "modified_at": self.ollama_server_infos.AtomRAG_CREATED_AT,
                        "details": {
                            "parent_model": "",
                            "format": "gguf",
                            "family": self.ollama_server_infos.AtomRAG_NAME,
                            "families": [self.ollama_server_infos.AtomRAG_NAME],
                            "parameter_size": "13B",
        ...
        async def generate(raw_request: Request, request: OllamaGenerateRequest):
            ...
            For compatibility purpose, the request is not processed by AtomRAG,
                    ...
                    async def stream_generator():
                        try:
                            ...
                            if isinstance(response, str):
                                ...
                                data = {
                                    "model": self.ollama_server_infos.AtomRAG_MODEL,
                                    "created_at": self.ollama_server_infos.AtomRAG_CREATED_AT,
                                    "response": response,
                                    "done": False,
                                }
        ...
        async def chat(raw_request: Request, request: OllamaChatRequest):
            ...
            Routes user queries through AtomRAG by selecting query mode based on prefix indicators.
