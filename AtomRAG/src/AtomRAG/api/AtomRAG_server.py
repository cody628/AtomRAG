...
from src.AtomRAG.types import GPTKeywordExtractionFormat
...
mongo_database = config.get("mongodb", "database", fallback="AtomRAG")
...
def display_splash_screen(args: argparse.Namespace) -> None:
    """
    Display a colorful splash screen showing AtomRAG server configuration
    ...
    ASCIIColors.cyan(f"""
    ╔══════════════════════════════════════════════════════════════╗
    ║                   🚀 AtomRAG Server v{__api_version__}                  ║
    ...
    ASCIIColors.yellow(f"{ollama_server_infos.AtomRAG_MODEL}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="AtomRAG FastAPI Server with separate working and input directories"
    )
    ...
    parser.add_argument(
        "--key",
        type=str,
        default=get_env_value("AtomRAG_API_KEY", None),
        help="API key for authentication. This protects AtomRAG server against unauthorized access",
    )
    ...
    parser.add_argument(
        ...
        default=get_env_value(
            "SIMULATED_MODEL_NAME", ollama_server_infos.AtomRAG_MODEL
        ),
        help="Number of conversation history turns to include (default: from env or 3)",
    )
    ...
    ollama_server_infos.AtomRAG_MODEL = args.simulated_model_name
...
def create_app(args):
    ...
    api_key = os.getenv("AtomRAG_API_KEY") or args.key
    ...
    app = FastAPI(
        title="AtomRAG API",
        description="API for querying text using AtomRAG with separate storage and input directories"
        + "(With authentication)"
        if api_key
        else "",
    ...
    if args.llm_binding == "lollms" or args.embedding_binding == "lollms":
        from src.AtomRAG.llm.lollms import lollms_model_complete, lollms_embed
    if args.llm_binding == "ollama" or args.embedding_binding == "ollama":
        from src.AtomRAG.llm.ollama import ollama_model_complete, ollama_embed
    if args.llm_binding == "openai" or args.embedding_binding == "openai":
        from src.AtomRAG.llm.openai import openai_complete_if_cache, openai_embed
    if args.llm_binding == "azure_openai" or args.embedding_binding == "azure_openai":
        from src.AtomRAG.llm.azure_openai import (
            azure_openai_complete_if_cache,
            azure_openai_embed,
        )
    if args.llm_binding_host == "openai-ollama" or args.embedding_binding == "ollama":
        from src.AtomRAG.llm.openai import openai_complete_if_cache
    ...
    async def clear_documents():
        """
        Clear all documents from the AtomRAG system.

        This endpoint deletes all text chunks, entities vector database, and relationships vector database,
        effectively clearing all documents from the AtomRAG system.
